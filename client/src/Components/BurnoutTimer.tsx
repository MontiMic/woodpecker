...
export default function BurnoutTimer({ endTime, onExpire }: BurnoutTimerProps) {
    ...
    return (
        <div className="flex items-center gap-2">
            <svg 
                className={`w-5 h-5 ${isExpired ? 'text-red-500' : isLowTime ? 'text-yellow-500' : 'text-blue-500'}`}
                fill="none" 
                stroke="currentColor" 
                viewBox="0 0 24 24"
            >
            ...
            <span className={`font-medium ${isExpired ? 'text-red-600' : isLowTime ? 'text-yellow-600' : 'text-gray-700'}`}>
                {timeRemaining}
            </span>
        </div>
    );
